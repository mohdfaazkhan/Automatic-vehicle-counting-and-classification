...
writer = cv2.VideoWriter('final_result.avi', cv2.VideoWriter_fourcc(*'xvid'),4,(640,360))

class CameraStream(object):
    def __init__(self, src=0):
        #self.stream = cv2.VideoCapture("%s"%RSTP_protocal)
        self.stream = cv2.VideoCapture("bb.mp4")
...
if __name__ == '__main__':
    ...
    model = torch.hub.load('ultralytics/yolov5', 'custom', path='best (1).pt', force_reload=True) 
